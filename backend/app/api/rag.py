...
from app.core.access import get_accessible_knowledge_base_ids
...
async def rag_query(
    ...
    scoped_knowledge_base_ids: list[int] | None = accessible_knowledge_base_ids
    if knowledge_base_id is not None:
        ...
        knowledge_base = knowledge_base_result.scalar_one_or_none()
        if not knowledge_base:
            raise HTTPException(status_code=403, detail="你没有检索该知识库空间的权限")
        scoped_knowledge_base_ids = [knowledge_base.id]

    docs = retrieve_documents(
        query=query,
        limit=limit,
        knowledge_base_ids=scoped_knowledge_base_ids,
    )
