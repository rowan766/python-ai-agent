...
### cd backend && uv run uvicorn app.main:app --reload --port 8000
    ...
    uvicorn app.main:app：启动 backend/app/main.py 里的 app 对象
...
### 上传文件会去哪里
    ...
    代码位置：backend/app/api/rag.py
    文件会先被临时保存到 backend/uploads/ 目录
    保存方式：UPLOAD_DIR / file.filename

### 上传后为什么本地看不到文件
    上传完成后，系统会立即解析文件内容
    然后进入 finally 逻辑，把 uploads/ 里的临时文件删除
    删除代码在 backend/app/api/rag.py 里的 file_path.unlink()
...
### 真正长期保存的是什么
    ...
    代码位置：backend/app/core/rag/indexer.py

### 文件进入 Qdrant 之前做了什么
    代码位置：backend/app/core/rag/loader.py
...
### 检索时发生了什么
    代码位置：backend/app/core/rag/retriever.py
